...
void PollKeys()
{
	...
	{
		numSpheres = min(20, numSpheres + 1);
	}
	else if (X::IsKeyDown(X::Keys::DOWN))
	{
		numSpheres = max(1, numSpheres - 1);
	}

	//Poly2
	if (X::IsKeyDown(X::Keys::RIGHT))
	{
		polysize = min(3, polysize + 1);
	}
...
bool GameLoop(float deltaTime)
{
	...
	X::DrawScreenText("[ENTER] - Toggle Collision Resolution ON/OFF", 20, 0, 24, green);
...
bool Menu(float deltaTime)
{
	...
	sprintf_s(polysizeDisplay, "Polysize: %d", polysize);
	X::DrawScreenText(polysizeDisplay, 900, 0, 16, green);

	char polysize2Display[100];
	sprintf_s(polysize2Display, "Number of Spheres: %d", numSpheres);
	...
	X::DrawScreenText("[UP/DOWN]......NumSpheres +/-", 20, 60, 24, green);
	X::DrawScreenText("[RIGHT/LEFT]...Polysize +/-", 20, 90, 24, green);

	X::DrawScreenText("For Best Demo Results: Polysize = 2 NumSpheres = 20", 20, 150, 24, green);
